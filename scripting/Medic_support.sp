...
#define PLUGIN_DESCRIPTION "Medic_support"
#define PLUGIN_NAME "Medic_support"
#define PLUGIN_VERSION "0.8.1"
#define PLUGIN_WORKING "1"
#define PLUGIN_LOG_PREFIX "Medic_support"
...
public void OnPluginStart() {
	cvarEnable = CreateConVar("sm_medic_support_enabled", "1", "Enable Medic", FCVAR_NOTIFY);
	cvarMedic = CreateConVar("sm_medic_support_class", "support", "Class name of support", FCVAR_NOTIFY);
	cvarDelay = CreateConVar("sm_medic_support_delay", "8", "Delay for revive", FCVAR_NOTIFY);
	cvarMaxRevive = CreateConVar("sm_medic_support_rp", "1", "max revive point for revive", FCVAR_NOTIFY);
	cvarMaxDistance = CreateConVar("sm_medic_support_distance", "100", "Distance for revive", FCVAR_NOTIFY);
	cvarMedicItem = CreateConVar("sm_medic_support_item", "brassknuckles", "Item for revive", FCVAR_NOTIFY);
	cvarReviveScore = CreateConVar("sm_medic_support_score", "50", "Get score when revive", FCVAR_NOTIFY);
...
public Action:Event_PlayerPickSquad(Handle:event, const String:name[], bool:dontBroadcast) {
	...
	if(IsValidPlayer(client) && strlen(class_template) > 1) {
		...
		if (StrContains(sClass[client], sType, false) != -1) {
			...
			PrintCenterTextAll("Now %s is support.", user_name);
		}
	}
