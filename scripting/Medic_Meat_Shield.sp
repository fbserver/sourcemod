...
#define PLUGIN_DESCRIPTION "Medic_Meat_Shield"
#define PLUGIN_NAME "Medic_Meat_Shield"
#define PLUGIN_VERSION "0.8.1"
#define PLUGIN_WORKING "1"
#define PLUGIN_LOG_PREFIX "Medic_Meat_Shield"
...
public void OnPluginStart() {
	cvarEnable = CreateConVar("sm_medic_Meat_Shield_enabled", "1", "Enable Medic", FCVAR_NOTIFY);
	cvarMedic = CreateConVar("sm_medic_Meat_Shield_class", "Meat_Shield", "Class name of Meat_Shield", FCVAR_NOTIFY);
	cvarDelay = CreateConVar("sm_medic_Meat_Shield_delay", "2", "Delay for revive", FCVAR_NOTIFY);
	cvarMaxRevive = CreateConVar("sm_medic_Meat_Shield_rp", "10", "max revive point for revive", FCVAR_NOTIFY);
	cvarMaxDistance = CreateConVar("sm_medic_Meat_Shield_distance", "200", "Distance for revive", FCVAR_NOTIFY);
	cvarMedicItem = CreateConVar("sm_medic_Meat_Shield_item", "defib", "Item for revive", FCVAR_NOTIFY);
	cvarReviveScore = CreateConVar("sm_medic_Meat_Shield_score", "1000", "Get score when revive", FCVAR_NOTIFY);
...
public Action:Event_PlayerPickSquad(Handle:event, const String:name[], bool:dontBroadcast) {
	...
	if(IsValidPlayer(client) && strlen(class_template) > 1) {
		...
		if (StrContains(sClass[client], sType, false) != -1) {
			...
			PrintCenterTextAll("Now %s is Meat_Shield.", user_name);
		}
	}
