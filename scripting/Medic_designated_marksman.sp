...
#define PLUGIN_DESCRIPTION "Medic_designated_marksman"
#define PLUGIN_NAME "Medic_designated_marksman"
#define PLUGIN_VERSION "0.8.1"
#define PLUGIN_WORKING "1"
#define PLUGIN_LOG_PREFIX "Medic_designated_marksman"
...
public void OnPluginStart() {
	cvarEnable = CreateConVar("sm_medic_designated_marksman_enabled", "1", "Enable Medic", FCVAR_NOTIFY);
	cvarMedic = CreateConVar("sm_medic_designated_marksman_class", "designated_marksman", "Class name of designated_marksman", FCVAR_NOTIFY);
	cvarDelay = CreateConVar("sm_medic_designated_marksman_delay", "8", "Delay for revive", FCVAR_NOTIFY);
	cvarMaxRevive = CreateConVar("sm_medic_designated_marksman_rp", "1", "max revive point for revive", FCVAR_NOTIFY);
	cvarMaxDistance = CreateConVar("sm_medic_designated_marksman_distance", "100", "Distance for revive", FCVAR_NOTIFY);
	cvarMedicItem = CreateConVar("sm_medic_designated_marksman_item", "brassknuckles", "Item for revive", FCVAR_NOTIFY);
	cvarReviveScore = CreateConVar("sm_medic_designated_marksman_score", "50", "Get score when revive", FCVAR_NOTIFY);
...
public Action:Event_PlayerPickSquad(Handle:event, const String:name[], bool:dontBroadcast) {
	...
	if(IsValidPlayer(client) && strlen(class_template) > 1) {
		...
		if (StrContains(sClass[client], sType, false) != -1) {
			...
			PrintCenterTextAll("Now %s is designated_marksman.", user_name);
		}
	}
